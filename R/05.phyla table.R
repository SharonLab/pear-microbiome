...
ps.rarefied <- readRDS(file = "Data/rarefied.phyloseq")

sample_data(ps.rarefied)

data_bars <- ps.rarefied %>%
  tax_glom(taxrank = rank_names(ps.rarefied)[2]) %>%
...
phyla_table <- reshape2::dcast(data_bars, cd.ps.short ~ Phylum, value.var = "Abundance", fun.aggregate = sum)

phyla_table <- phyla_table %>%
  separate(cd.ps.short, into = c("Date", "Stage"), sep = " ", remove = FALSE, extra = "merge")
...
phyla_table[1:ncol(phyla_table)] <- round(phyla_table[1:ncol(phyla_table)], 4)
